...
def _buscar_metodos_agente(nome_agente):
    ...
    # fuzzy, que casava SILICA dentro de SILICATO (ver `_palavra_inteira` abaixo).
    ...
    def _palavra_inteira(agulha, palheiro):
    ...
    for nome_upper, cas in guia.get('by_name', {}).items():
        ...
        if _palavra_inteira(key_sa, _sa(nome_norm)):
    ...
    for nome_upper, cas in guia.get('by_name', {}).items():
        nome_norm = _norm(nome_upper)
        if len(nome_norm) > 6 and _palavra_inteira(_sa(nome_norm), key_sa):
            if best is None or len(nome_norm) > len(best[0]):
                best = (nome_norm, cas)
    if best:
        return guia.get('by_cas', {}).get(best[1], [])
